...
# i_end = right border, i_start = left border of ambiguosity
x_l = x[i_start]
x_r = x[i_end]

def countArea(xs, ys): # using Gauss's algotithm
    # xs is array of x, ys is array of u (in this case)
    area = 0
    n = len(xs)
    for i in range(n - 1):
        area += xs[i] * ys[i + 1]
...
min_diff = x[len(x) - 1] - x[0] # just big number; will be replaced by other calculations
for i in range(len(x) - 1):
    if abs(x[i + 1] - x[i]) < min_diff:
        min_diff = abs(x[i + 1] - x[i])

area_r = 2 * abs((x[len(x) - 1] - x[0]) * (u[len(u) - 1] - u[0]))
area_l = 0
newArea_r = abs((x[len(x) - 1] - x[0]) * (u[len(u) - 1] - u[0]))
newArea_l = 0

while x_r - x_l > min_diff / 2:
    area_r = newArea_r
    area_l = newArea_l
    x_mid = 0.5 * (x_l + x_r)
    xs = []
    ...
    for i in range(i_start):
        if x[i] >= x_mid:
            xs.append(x[i])
            ys.append(u[i])
    area_r = countArea(xs, ys)
    xs.clear()
    ys.clear()
    for i in range(i_end, len(x)):
        if x[i] <= x_mid:
            xs.append(x[i])
            ys.append(u[i])
    area_l = countArea(xs, ys)
    if area_r > area_l:
        x_l = x_mid
    elif area_r < area_l:
        x_r = x_mid
